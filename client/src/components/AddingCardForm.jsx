...
export const AddingCardForm = (props) => {
  ...
  const title_ExistingProduct = "Product had already existed!";
  ...
  const onAddClickHandle = async () => {
    ...
    const { data: fetchedProduct } = await fetchProduct(props.productId);

    // check if fetched Product is existed
    if (fetchedProduct.length !== 0) {
      // if fetched Product is existed
      ...
      // if fetched Product is NOT existed
